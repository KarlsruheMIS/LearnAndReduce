...
inline void general_reduction::get_neighborhood_vector(NodeID v, branch_and_reduce_algorithm *br_alg, sized_vector<NodeID> &vec)
{
    auto &status = br_alg->status;
    vec.clear();
...
inline bool fold2_reduction::reduce_vertex(branch_and_reduce_algorithm *br_alg, NodeID v)
{
    ...
    if (v_shape_min_count >= status.modified_stack.capacity() - status.n)
    { // initial cpapcity is 2n (one n for v_shape min)
...
void fold2_reduction::restore(branch_and_reduce_algorithm *br_alg)
{
    ...
    case fold2_reduction::fold2_case::v_shape_mid:
        br_alg->unset(deg2_node);
        if (status.node_status[nb0] != IS_status::not_set)
            br_alg->unset(nb0, false);
        if (status.node_status[nb1] != IS_status::not_set)
    ...
    case fold2_reduction::fold2_case::v_shape_min:
        if (status.node_status[deg2_node] != IS_status::not_set)
...
bool funnel_fold_reduction::is_funnel(NodeID v, NodeID &funnel_neighbor, branch_and_reduce_algorithm *br_alg, fast_set &funnel_set, sized_vector<NodeID> &funnel_nodes)
{
    ...
    for (NodeID u : funnel_nodes)
    {
        ...
        if (std::any_of(funnel_nodes.begin(), funnel_nodes.end(), [&](NodeID neighbor)
                        { return weights[neighbor] + weights[u] < weights[v]; }))
        {
