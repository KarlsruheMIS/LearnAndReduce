...
bool bound_reduction::reduce(reduce_algorithm* br_alg) {
...
NodeWeight bound_reduction::get_lower_bound(reduce_algorithm* br_alg, graph_access& G, fast_set& lb_solution)
{
...
void bound_reduction::reduce_component(reduce_algorithm* br_alg, graph_access& G) {
...
void bound_reduction::reduce_by_clique_cover_bound(reduce_algorithm* br_alg, graph_access& G, NodeWeight lb, NodeWeight ub, std::vector<NodeID>& vertices_to_include, std::vector<NodeID>& reverse_mapping, int n_cliques)
{
    auto& config = br_alg->config;
    auto& status = br_alg->status;
...
NodeWeight bound_reduction::get_clique_cover_bound(reduce_algorithm* br_alg, int& n_cliques, fast_set& solution, std::vector<NodeID>& vertices_to_include, std::vector<NodeID>& reverse_mapping)
{
    auto& status = br_alg->status;
    auto& buffers = br_alg->buffers;
	auto &clique = buffers[2];
...
NodeWeight bound_reduction::compute_cover_pruning_bound(reduce_algorithm* br_alg, int& n_cliques, std::vector<NodeID>& reverse_mapping)
{
    auto& buffers = br_alg->buffers;
    auto& status = br_alg->status;
...
inline bool bound_reduction::reduce_vertex(reduce_algorithm* br_alg, NodeID v) {
    return false;
}
