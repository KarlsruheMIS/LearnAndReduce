...
inline bool heavy_vertex_reduction::reduce_vertex(branch_and_reduce_algorithm *br_alg, NodeID v)
{
    ...
    if (config.disable_heavy_vertex)
        return false;
    if (br_alg->deg(v) < 3)
        return false; // reduce with fold1 or 2
    ...
    if (br_alg->deg(v) > config.subgraph_node_limit)
        return false; // subgraph too large
...
bool path_reduction::dequeue_node(std::vector<NodeID> &queue, NodeID &n, size_t degree) {
    for (;;) {
        ...
        n = queue.back();
        queue.pop_back();
        if (br_alg->status.node_status[n] == IS_status::not_set && br_alg->deg(n) == degree)
            return true;
    }
}
...
{
    NodeID n;
    if (!dequeue_node(br_alg->buffers[0], n, 1))
        return false;
...
{
    NodeID n;
    if (!dequeue_node(br_alg->buffers[1], n, 2))
        return false;
