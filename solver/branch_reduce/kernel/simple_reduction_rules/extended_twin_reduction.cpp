...
bool extended_twin_reduction::reduce(branch_and_reduce_algorithm *br_alg)
{
...
inline bool extended_twin_reduction::reduce_vertex(branch_and_reduce_algorithm *br_alg, NodeID v)
{
    auto &status = br_alg->status;
...
void extended_twin_reduction::fold(branch_and_reduce_algorithm *br_alg, NodeID v, NodeID cand)
{
...
void extended_twin_reduction::restore(branch_and_reduce_algorithm *br_alg)
{
    auto &status = br_alg->status;
...
void extended_twin_reduction::apply(branch_and_reduce_algorithm *br_alg)
{
    auto &status = br_alg->status;
...
int extended_twin_reduction::generate_data(branch_and_reduce_algorithm *br_alg, NodeID v, std::vector<NodeID> &label)
{
    auto &status = br_alg->status;
    auto &neighbors = br_alg->buffers[0];
    auto &candidate_neighbors = br_alg->buffers[1];
