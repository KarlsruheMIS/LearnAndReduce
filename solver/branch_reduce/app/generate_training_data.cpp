...
bool write_reduction_data_csv(graph_access &G, std::vector<std::vector<bool>> &reduction_data,
                              std::string filename, sized_vector<std::string> &reduction_names,
...
int main(int argn, char **argv)
{
    ...
    for (int i = 0; i < config.num_of_subgraphs; i++)
    {
        ...
        write_reduction_data_csv(subgraph, reduction_data, "training_data/csv/" + name + "_seed" + std::to_string(config.seed) + "_training_data_graph_" + std::to_string(i), reduction_names, exclude_data, include_data);
