...
bool extended_struction<reduced>::reduce(reduce_algorithm* r_alg, NodeID n, size_t max_nodes) {
    this->r_alg = r_alg;
    auto &status = r_alg->status;

    auto &additional_nodes = r_alg->buffers[1];
    auto &layer_prefix_count = r_alg->buffers[2];
    auto &tmp_marks = r_alg->set_1;
    if (!finder.findAllMWIS<reduced>(r_alg, status.graph[n], status.weights[n], max_nodes)) return false;

    std::vector<mwis> &sets = finder.get_sets();

    if (reduced && !findAdditionalNodes(n, sets, r_alg->buffers[3], layer_prefix_count, additional_nodes, max_nodes - sets.size())) {
        return false;
    }

    // Create set nodes
    const NodeID first_set_node = status.graph.size();
    r_alg->push_nodes(sets.size());
    ...
    if (reduced) {
        ...
        r_alg->push_nodes(additional_nodes.size());
    ...
    r_alg->set(n, IS_status::folded, true);
    for (const auto u : status.graph[n])
            r_alg->set(u, IS_status::folded, false);
...
void extended_struction<reduced_version>::restore(reduce_algorithm* r_alg) {
    auto &status = r_alg->status;
    auto &data = restore_vec.back();

    // remove new nodes
    r_alg->pop_nodes(data.set_nodes);
    ...
    r_alg->unset(data.main);
    status.reduction_offset -= status.weights[data.main];
    restore_vec.pop_back();
}

template<bool reduced_version>
void extended_struction<reduced_version>::apply(reduce_algorithm* r_alg) {
    auto &status = r_alg->status;
    auto &data = restore_vec.back();
...
template<bool reduced_version>
size_t extended_struction<reduced_version>::removed_vertices(reduce_algorithm* r_alg, NodeID n) {
    return r_alg->deg(n) + 1;
}
