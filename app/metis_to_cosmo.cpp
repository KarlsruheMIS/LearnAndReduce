...
int write_weights_and_reductions_to_csv(graph_access & G, const std::string & resulting_filename, const std::string & filename, ReductionConfig & config) {
    ...
    if (!config.disable_simplicial_vertex) reduction_names.push_back("clique");
    ...
    if (!config.disable_single_edge) reduction_names.push_back("single_edge");
    if (!config.disable_extended_single_edge) reduction_names.push_back("extended_single_edge");
    ...
    if (!config.disable_struction_decrease) reduction_names.push_back("decreasing_struction");
