...
struct high_degree_reduction : public general_reduction
{
    ...
    bool reduce_in_component(reduce_algorithm *br_alg, NodeID v, graph_access &G, NodeWeight &MWIS_weight);
...
private:
    ...
    void fold(reduce_algorithm *br_alg, fold_nodes &nodes, NodeWeight MWIS_weight, std::vector<NodeID> &excluded_nodes, std::vector<bool> &component_nodes);
