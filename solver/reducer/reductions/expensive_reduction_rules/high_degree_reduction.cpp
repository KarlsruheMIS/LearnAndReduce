...
bool high_degree_reduction::reduce(reduce_algorithm* br_alg) {
...
bool high_degree_reduction::reduce_in_component(reduce_algorithm* br_alg, NodeID v, graph_access& G, NodeWeight& MWIS_weight) {
	auto& status = br_alg->status;
...
void high_degree_reduction::fold(reduce_algorithm* br_alg, fold_nodes& nodes, NodeWeight MWIS_weight, std::vector<NodeID>& excluded_nodes, std::vector<bool>& component_nodes) {
	auto& status = br_alg->status;
...
void high_degree_reduction::restore(reduce_algorithm* br_alg) {
	auto& status = br_alg->status;
...
void high_degree_reduction::apply(reduce_algorithm* br_alg) {
	auto& status = br_alg->status;
