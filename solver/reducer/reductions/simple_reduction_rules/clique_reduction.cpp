...
inline bool clique_reduction::reduce_vertex(reduce_algorithm *br_alg, NodeID v)
{
    auto &status = br_alg->status;
    auto &set_1 = br_alg->set_1;
    auto &neighbors = br_alg->buffers[0];
...
void clique_reduction::fold(reduce_algorithm *br_alg, const weighted_node &isolated, std::vector<NodeID> &&non_isolated)
{
    auto &status = br_alg->status;
...
void clique_reduction::restore(reduce_algorithm *br_alg)
{
    auto &status = br_alg->status;
...
void clique_reduction::apply(reduce_algorithm *br_alg)
{
    auto &status = br_alg->status;
...
int clique_reduction::generate_data(reduce_algorithm *br_alg, NodeID v, std::vector<NodeID>& label)
{
    auto &status = br_alg->status;
    auto &set_1 = br_alg->set_1;
    auto &neighbors = br_alg->buffers[0];
