...
ils::ils(const Config &config) : config(config), local(config){
...
void ils::perform_ils(graph_access& G, unsigned int iteration_limit, int offset) {
  ...
  // set inital soluation to best solution
    ...
    while (iterations < iteration_limit) {
        ...
        v = best_v;
        assert(v != -1 && "No vertex found for forceful insertion");
...
void ils::force(Config &config, graph_access &G, NodeID v,
                candidate_list *force_list, bool with_candidates) {
  // printf("Force %d\n", v);
  if (perm->is_solution_node(v))
    printf("Vertex already in the solution.\n");
  if (force_list) {
  ...
  if (with_candidates) {
  if (cand && !cand->contains(v))
    local.add_candidate(v, G);
}
}
